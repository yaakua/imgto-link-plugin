    ...
    try {
        ...
        if (cachedUser && cachedUser.loggedIn) {
            ...
            if (cacheAge < maxAge && cachedUser.username) {
                ...
                if (userCookie && userCookie.value) {
                    console.log('[FaFaFa-全部发] HuaweiDev: using cached user info:', cachedUser.username)
                ...
                // cookie 已失效，清除缓存
                console.log('[FaFaFa-全部发] HuaweiDev: cache exists but cookie gone, clearing cache')
        ...
        if (tabs.length > 0) {
            try {
                ...
                console.log('[FaFaFa-全部发] HuaweiDev: executeScript failed:', e.message)
            }
        }
        ...
        const userCookie = await chrome.cookies.get({ url: 'https://developer.huawei.com', name: 'developer_userdata' })
        if (userCookie && userCookie.value) {
            console.log('[FaFaFa-全部发] HuaweiDev: developer_userdata cookie found, trying offscreen fetch for user info')
            let username = ''
            let avatar = ''
            ...
            try {
                ...
                console.log('[FaFaFa-全部发] HuaweiDev: offscreen fetch failed:', e.message)
            }
            ...
            if (apiSuccess) {
                ...
                // API 失败，清除可能残留的缓存
                await chrome.storage.local.remove('huaweidev_user')
                console.log('[FaFaFa-全部发] HuaweiDev: API verification failed, treating as logged out')
        ...
        console.error('[FaFaFa-全部发] HuaweiDev Detection Error:', e)
