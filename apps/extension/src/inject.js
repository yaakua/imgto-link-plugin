      ...
      try {
        if (error) {
          ...
          if (error.includes && error.includes('Extension context invalidated')) {
            console.warn('[FaFaFa-全部发] 扩展已重新加载，请刷新页面')
        ...
        console.warn('[FaFaFa-全部发] 扩展上下文已失效，请刷新页面')
  ...
  const publisherBridge = {
    ...
    getAccountsProgressive(onProgress, onComplete) {
      ...
      sendMessage('CHECK_PLATFORM_STATUS_PROGRESSIVE', { platforms: PLATFORMS })
        .catch(error => {
          console.error('[FaFaFa-全部发] 渐进式检测启动失败:', error)
          // 如果启动失败，调用完成回调
            ...
            try {
              ...
              for (const item of clipboardItems) {
                if (item.types.includes('text/html')) {
                  ...
                  console.log('[FaFaFa-全部发] 已读取剪贴板 HTML 内容，长度:', clipboardHtmlContent.length)
                  break
                }
              }
            } catch (e) {
              console.log('[FaFaFa-全部发] 读取剪贴板失败:', e.message)
            }
          }
        }
